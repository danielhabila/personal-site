...
import BgImage from "../images/podcast-single.png";
...
function AudioPlayer(props, ref) {
  ...
  const [outputDuration, setOutputDuration] = useState("0:00");
  const [currentTime, setCurrentTime] = useState(0);
  const [outputCurrentTime, setOutputCurrentTime] = useState("0:00");
  const [progress, setProgress] = useState(0);
  const [isDownEvent, setIsDownEvent] = useState(false);
  const speedOptions = [1, 1.5, 2];
  const speedOutput = useRef(["1x", "1.5", "2x"]);
  ...
  useEffect(() => {
    audio.current.playbackRate = speedOptions[speedIndex];
  }, [speedIndex]);
  ...
  const updateBarPosition = (e) => {
    ...
    const pos =
      (x -
        (slider.current.getBoundingClientRect().x ||
          slider.current.getBoundingClientRect().left)) /
      slider.current.getClientRects()[0].width;
    audio.current.currentTime = (duration * keepRange(pos) * 100) / 100;
  };
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="pb-12 md:pb-20">
          <div className="relative">
            {/* Background */}
            <div
              className="absolute inset-0 bg-gradient-to-br from-slate-900 to-slate-800  max-md:-mx-20 rounded-3xl -z-10"
              aria-hidden="true"
            >
              <img
                  ...
                  >
                    ...
                    <div className="sm:flex items-center sm:space-x-3">
                      ...
                      <div className="grow flex items-center space-x-3">
                        ...
                        >
                          <div
                            className="h-2 bg-gradient-to-bl from-white pointer-events-none"
                        ...
                        <div className="flex items-center text-xs font-medium text-white space-x-1.5 tabular-nums">
                          ...
                          <span
                            className="w-px h-4 bg-white bg-opacity-30"
                            aria-hidden="true"
                          ></span>
